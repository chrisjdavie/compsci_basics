...
class Test(unittest.TestCase):
    ...
    def test_pivot(self, _name, arr, target, _, pivot):
        # the definition of pivot here is ambiguous, so I'm defining it as
        # the peak
        self.assertEqual(_find_pivot(arr), pivot)

    @parameterized.expand([
        ("present", [1, 3, 5, 7], 5, 2),
        ("not present", [1, 3, 5, 7], 4, -1)
    ])
    def test_binary_search_ascending(self, _, arr, target, expected_result):
        self.assertEqual(binary_search(arr, target), expected_result)

    @parameterized.expand([
        ("present", range(19, 0, -2), 17, 1),
        ("not present", range(19, 0, -2), 18, -1)
    ])
    def test_binary_search_descending(self, _, arr, target, expected_result):
        self.assertEqual(binary_search(arr, target, sign=-1), expected_result)

    @parameterized.expand(TEST_DATA)
    def test_result(self, _name, arr, target, expected_index, _):
...
def _find_pivot(arr):
    ...
    # not checking validity of array
    for _ in range(end+1):
        ...
        if midpoint <= 0:
            break
        if arr[midpoint] > arr[midpoint-1] and arr[midpoint+1] < arr[midpoint]:
            break
...
def binary_search(arr, target, start=0, end=None, sign=1):
    if end is None:
        end = len(arr) - 1
    ...
    raise InvalidValueError("Code should never get here")
...
def find_number(arr, target):
    ...
    pivot = _find_pivot(arr)

    lhs_result = binary_search(arr, target, 0, pivot-1)
    rhs_result = binary_search(arr, target, pivot, len(arr)-1, -1)
    if lhs_result > -1:
        return lhs_result
    if rhs_result > -1:
        return rhs_result
