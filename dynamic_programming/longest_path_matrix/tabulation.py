...
def longest_path(matrix):
    ...
    results = [ [None]*len(matrix) for _ in range(len(matrix)) ]

    # final row
    for j in range(len(matrix)):
        results[-1][j] = matrix[-1][j]
    # each row towards the beginning
    for i in range(2, len(matrix) + 1):
        for j in range(len(matrix)):
            prev_results = [ results[-(i-1)][j] ]
            if j-1 >= 0:
                prev_results.append(results[-(i-1)][j-1])
            if j+1 < len(matrix):
                prev_results.append(results[-(i-1)][j+1])
            results[-i][j] = max(prev_results) + matrix[-i][j]
