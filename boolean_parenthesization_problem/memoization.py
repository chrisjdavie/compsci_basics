...
def memoize(func):
    ...
    def memoized_func(statement, ind_start, ind_end):
        nonlocal results
        if ind_start ==0 and ind_end == len(statement):
            results = {}

        key = (ind_start, ind_end)
        if key not in results:
            results[key] = func(statement, ind_start, ind_end)
        return results[key]
...
def evaluate_statement(statement):        
    ...
    return _evaluate_statement(statement, 0, len(statement))[0]%1003
